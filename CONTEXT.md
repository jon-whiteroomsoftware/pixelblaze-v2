...
_See also_: **Solidity** — a curved surface that supplies a **surface normal** (cylinder, sphere, surface-cube) is **solid-eligible**; the flat identity surface is not (it faces the camera).
...
**Solidity** (ADR-0011):
A preview-only, **per-pattern** display property of a **surface** embedding: a continuous slider `0 = transparent → 1 = solid` that suppresses **back-facing** points so a solid object hides its own back, while `0` is exactly today's see-through draw (the points hung on glass / wire mesh / in free space). A property of *surfaces only* — defined by the presence of a per-point **surface normal**: curved embeddings (Pole, Cylinder, Sphere, surface-cube / polyhedron nets) supply one and are **solid-eligible**; flat ones (Line, Ring, Flat) trivially face the camera; **volumes** (volumetric cube, a measured cloud) have no normal and are never eligible. (This is why "cube" splits in two: a **surface cube** — LEDs on six faces, face normals, eligible — vs a **volumetric cube** — points filling space, not eligible.) Eligibility is **provenance-gated**, not geometry-inferred: a normal is available only because the IDE owns the generator (analytic embeddings emit it from their formula; the **3D Sphere stock map** carries a `solidEligible` flag and the preview re-derives `normalize(pos − centroid)`), so a hand-imported sphere-shaped cloud is never solid-able — the **Stock map** vs **Custom map** provenance split, applied to normals. Mechanically a soft **terminator fade**: a `normal · viewDir` brightness multiplier folded into `project3D` beside the **depth cue** (a geometric visibility factor, *not* the **brightness** control — front-facing points are never touched; the slider sets the floor the back fades to). Default `1.0`; a **recommended solidity** (`demoName → number`, the recommended-map / recommended-count registry family) sets a **demo**'s on-open value. Persisted on `PatternRecord` beside `surfaceId`; **never** serialized to a **controller** (per-pattern and hardware-bound are orthogonal axes).
...
The per-point outward unit vector a solid-eligible embedding supplies so the preview can compute **solidity**. Preview-only — re-derived from the embedding's generator (analytic formula, or `normalize(pos − centroid)` for a flagged stock shell), **never** stored in a **map** record or sent to a **controller** (a Pixelblaze map is positions only). Absence of a normal is exactly what makes an embedding solid-ineligible.
