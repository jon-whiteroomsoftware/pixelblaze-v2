...
**Extension**:
A browser extension the user installs that lets the deployed (GitHub Pages, https) IDE reach a Controller. The page itself cannot open the Controller's `ws://` socket directly (mixed-content blocking), but the extension — running with elevated privileges outside the page's mixed-content sandbox — can, relaying frames between the IDE and the Controller and handling discovery. Optional, local-only, and purely additive — authoring/preview/export work with no extension installed. The IDE never launches the extension; the user installs it and the IDE detects it.
