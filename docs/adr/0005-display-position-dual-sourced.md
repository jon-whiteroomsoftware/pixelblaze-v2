...
- **The 1D shape persists per-pattern** on `PatternRecord` alongside `mapId` (no `DB_VERSION` bump — schemaless record), so a strip wrapped into a helix reopens as that helix. **Preview light size** and **diffusion** stay global viewing-comfort prefs; camera angle is ephemeral. (The "spacing" control referenced in earlier drafts was renamed and redefined — it never moved the dots apart but only scaled their drawn size; see [ADR-0006](0006-preview-light-size-and-diffusion.md).)
