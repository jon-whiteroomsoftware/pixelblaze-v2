...
- **Uniformly rescaling a regular cloud is invisible to the pattern** — normalization scales it right back into 0..1. (This is why "spacing" is a *display* concern; see §5.)
...
### 5. Spacing and (for 1D) shape are viewport concerns

Because of §4, how far apart the dots sit is a property of the **camera/viewport**, not the map — moving them uniformly closer or farther changes the picture, never the pattern's input. So a "spacing" control is a single uniform camera scale, and it works identically in 1D, 2D, and 3D for the same reason: it spreads the dots along the line, across the XY plane, or through the volume, and `sample` never moves because §4 normalizes each axis from the *counts*, not the spacing.
...
- **Spacing slider** — a single uniform camera scale (§5), display-only and pattern-invisible. It spreads the dots along the line (1D), across the XY plane (2D), or through the volume (3D) by the same scalar. Auto-fit-to-container (`Preview.tsx` `ResizeObserver`) still frames whatever cloud results; spacing rides on top.
- **Diffusion slider** — the existing isotropic `diffusion` CSS-blur on the WebGL canvas (glow / light-bleed approximation), unchanged and meaningful in every dimension. In 3D it blurs the final projected image.
...
- **No scroll-dolly** — the spacing slider + auto-fit cover sizing; a separate camera-distance zoom is intentionally omitted (an easy future add if dense clouds prove fiddly).
...
- **1a — the architecture pivot, no 3D viewport.** Map concept in the stores; explicit-positions spatial layer (WebGL renderer + pure `camera.ts`) revealed **locked-2D**; the current grid re-expressed as the default stock 2D map; render-fn detection + native-dimensionality derivation; dispatch by active-layout `sample`-arity via the fallback chain; 1D **viewport shape embeddings** (line, ring; polygon if room) with path display on the WebGL canvas; the "Shape" dropdown; title-bar dimensionality; uniform spacing + diffusion as camera controls. **Ships looking like today's grid, plus real 1D.** This is the safe tracer bullet — a refactor whose 2D output should be *visually* unchanged (coordinate-identical; the draw moves Canvas-2D → WebGL).
...
- **Pure camera/projection module**: `pos` → clip coords, orbit matrix, depth scaling, fit-to-container, uniform spacing scale — including the locked-2D camera mapping the default grid to the expected screen layout (no canvas needed).
...
src/
  engine/
    camera.ts          → NEW pure projection/orbit/depth/fit/spacing-scale
  ...
  store/
    ...
    previewStore.ts    → GridConfig demoted to default seed; global spacing +
                         diffusion; ephemeral camera state                        (Phase 1)
  components/
