...
The controller supports the fast, high dynamic range APA102/SK9822 (AKA Dotstar) LEDs, as well as WS2801, and WS2811/WS2812/WS2813/WS2815/GS8208/SK6812 (AKA NeoPixel) LEDs.

### Recommended: SK9822 Type LEDs

The APA102/SK9822 (AKA Dotstar) LEDs are recommended for their beautiful HDR color and fast clock speeds. The modules usually have 4 input pins or wires: 2 for power, 1 for clock, and one for data. These can come in a wide variety of styles and markings. These have integrated controllers, so you won't see a separate IC controlling the LEDS, it's built in. Here are some pictures of various strips:

![](https://electromage.com/img/strip1.jpg) ![](https://electromage.com/img/strip2.jpg) ![](https://electromage.com/img/strip3.jpg)

### WS2812 Type LEDs

WS2812 (AKA NeoPixel) and compatible LEDs like the WS2811, WS2813, WS2815, GS8208, and SK6812 are also supported. Some of these require 12V, and in these setups will require separate 5V power for Pixelblaze either via the VIN/GND pins or USB. Never connect 12V to the Pixelblaze's VIN pin.
...
Unless an LED is listed as compatible with one of the above variants, it may not work with Pixelblaze. If you are unsure about what LED chipset you have, post a photo to the [forum](https://forum.electromage.com/) and we'll try to help identify it!
...
Some LEDs draw less. Variants that draw ~12mA per channel (36mA for all 3) are common. There are low power variants that draw up to 5mA per channel. Some 12V LEDs like the WS2815 and GS8208 connect each element in series such that multiple output elements do not increase current consumption.
...
![](https://electromage.com/img/V3_Standard_USB.jpg)
...
![](https://electromage.com/img/v3_standard_No_USB_.jpg)
...
### Connecting APA102 / SK9822 Type LEDs (4 wire)
...
![](https://electromage.com/img/v3_standard_strip1.jpg)
...
![](https://electromage.com/img/v3_standard_strip2.jpg)
...
![](https://electromage.com/img/v3pico_direct.jpg)
...
### Connecting LEDs with a backup data line

If your LEDs supports a backup data channel, typically labeled BI or DI2, it should be connected to GND. Keep the wire handy — if the first LED dies you can swap it to DAT to bypass the first pixel. You may choose to connect BI to DAT anyway, but some types of LEDs with a backup input get confused when a data signal arrives on BI before the first LED has forwarded some data.

### Connecting WS2812 / Neopixel Type LEDs (3 wire)
...
### Connecting WS2815 or GS8208 Type LEDs (4 wire, DAT and backup DAT)

This configuration will use 2 of the available connections on the Pixelblaze's LED connection header: DAT (data), and GND. They must always be connected to the LEDs to function.

VIN must NOT be connected to the positive 12 V conductor for your LEDs. The controller must be powered off a separate 5 V power supply, or by using a small [12 V -> 5 V DC-DC buck converter](https://shop.electromage.com/products/mini-buck-12v-to-5v-dc-dc-converter). The CLK signal is not used and should be left unconnected.
...
Pixelblaze has several [GPIOs](https://electromage.com/docs/GPIO) that can be used as digital or analog inputs, or as outputs to drive external circuitry.

All of the GPIO are 3.3 V, with the exception of the ADC input on V2/V2+, which has a limit of 1.0 V.