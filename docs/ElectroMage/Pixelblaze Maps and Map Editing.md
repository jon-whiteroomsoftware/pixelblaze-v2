...
1.  [Pixel Mapping System Overview](#pixel-mapping-system-overview)
2.  [Coordinate World Units](#coordinate-world-units)
3.  [Using the Map Editor](#using-the-map-editor)
4.  [Map Using a JSON Array](#map-using-a-json-array)
5.  [Map Using JavaScript](#map-using-javascript)
6.  [Rendering with Pixel Maps](#rendering-with-pixel-maps)
...
Note that this JavaScript is run on your browser, and not the same as Pixelblaze's language. Many of Pixelblaze's shortcuts are not available, e.g. use `Math.cos()` instead of `cos()`. Any code you write here is only evaluated once in order to produce the pixel map. The function can take a `pixelCount` argument.
